...
function onCreate()
 if difficultyName == 'Hard' then
    ...
    makeAnimatedLuaSprite('BotonPendulo', 'botonPendulo', 1100, 145);
...
if keyJustPressed('space') and yourtriallife > -1 or (getMouseX('camHUD') > 1100 and getMouseX('camHUD') < 1250) and (getMouseY('camHUD') > 145 and getMouseY('camHUD') < 275 and mouseClicked('left')) and yourtriallife > -1 then 
...
if curBeat % 1 == 0 then
   -- debugPrint(getProperty('lightblueBar.scale.y'))  -- i can see the value shit
    setProperty('health',getProperty('health')- 0.08 * getProperty('lightblueBar.scale.y')*1.4)
...
function goodNoteHit()--Con esto se compensa la mecánica
if difficultyName == 'Hard' then
